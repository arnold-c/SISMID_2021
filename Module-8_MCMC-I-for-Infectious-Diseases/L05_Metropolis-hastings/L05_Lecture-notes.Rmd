...
-   If you have an inrredicible Markov chain ($\{X_n\}$) and positive recurrent
    ...
    -   $\underset{N \to \infty}{\lim} \underbrace{\frac{1}{N} \sum_{k=1}^N f(X_k)}_{\text{time average}} = \sum_{i \in E} f(i) \pi_i = \underbrace{E_\pi[f(X)]}_{\text{space average}}$
...
2.  **for** $n=0 \to N$:
    ...
    1.  Simulate candidate value
        $Y \sim \underbrace{q(j|X_n = i)}_{\substack{\text{proposal transition} \\ \text{ density / jumping density}}}$
        Suppose $Y=j$
