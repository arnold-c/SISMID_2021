...
unif_rw_next = function(cur_value, tuning_par){
  ...
  U1 <- cur_value + runif(1, -tuning_par, tuning_par)
  U2 <- runif(1, min = 0, max = 1)
  
  if (U2 < dnorm(U1)/dnorm(cur_value)){
    return_value <- c(U1, 1)
  }
...
for (i in 2:mcmc_size){
  mcmc_out[i,] = unif_rw_next(mcmc_out[i-1, 1], tuning_par = 5)# using row mcmc_out[i-1,] and unif_rw_next
}
